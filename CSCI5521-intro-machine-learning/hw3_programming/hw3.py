...
for i, num_hid in enumerate(candidate_num_hid):
    ...
    print('Validation accuracy for %d hidden units is %.3f' %
          (candidate_num_hid[i], cur_valid_accuracy))
...
accuracy = np.count_nonzero(predictions.reshape(-1)
                            == test_y.reshape(-1))/len(test_x)
